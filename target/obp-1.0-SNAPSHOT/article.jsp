    ...
    <div id="bbsArticle_footer">
        <div id="leftFooter">
            ...
            <input type="button" value=" 삭제 " class="btn2"
                   onclick="javascript:location.href='<%=cp%>/deleted_ok.jsp?num=<%=dto.getNum()%>&pageNum=<%=pageNum%><%=param%>';"/>
        </div>
        <div id="rightFooter">
            <input type="button" value=" 리스트 " class="btn2"
                   onclick="javascript:location.href='<%=cp%>/list.jsp?pageNum=<%=pageNum%><%=param%>';"/>
            <!-- 여기 pageNum을 넣어줘야함 -->
        </div>
    </div>
